...
const StyledMaterialTextInput = withProps<any, HTMLDivElement>(styled.div)`
  position: relative;
  label {
    position: absolute;
    left: 0;
...
const MaterialTextInput = (props: IProps) => {
  const {
    name,
    value,
    onChangeInput,
  ...
  return (
    <StyledMaterialTextInput isEmpty={props.value.length === 0}>
      <input 
        type="text" 
        id={name} 
        name={name} 
